...
# This script has been generated by AI copilot

set -e

# Configuration
TEST_DIR="tests/cosmo_numba"
...
for test_file in $TEST_FILES; do
    ...
    if [ $CURRENT -eq 1 ]; then
        # First run: create new coverage file
        COVERAGE_MODE=1 coverage run --source=./src -m pytest -sv "$test_file"
    else
        # Subsequent runs: append to existing coverage file
        COVERAGE_MODE=1 coverage run --append --source=./src -m pytest -sv "$test_file"
    fi
...
# Generate reports (no need to combine!)
echo "=========================================="
