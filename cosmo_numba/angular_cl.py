...
def get_k_bound(chi1, chi2, ell):
    ...
    if chi_min <= 0.:
        chi_min = 0.5*(ell+0.5)/1000.

    lkmax = np.log(min(1000., 2*(ell+0.5)/chi_min))
...
def cl_integ(ell_arr, chi2a_interp, tracer1, tracer2, pk_interp, cl):
    tracer1_interp = AkimaInterp1D(
        tracer1[0].astype('float64'),
        tracer1[1].astype('float64')
    )
    tracer2_interp = AkimaInterp1D(
        tracer2[0].astype('float64'),
        tracer2[1].astype('float64')
    )
