...
def H_p(t, t_int, B, t_bar):
    return 1/(8*B**3) * \
        (4*B**2 + 3*((t/t_bar)**2 - 1 - B**2) * ((t_int/t_bar)**2 - 1 - B**2))
...
def H_m(t, t_int, B, t_bar):
    return (1-B)**2/(8*B**3) * \
        (
            3*(1-B)**2 * ((1+B)**4-(1+4*B+B**2)*(t/t_bar)**2) *
            (t_int/t_bar)**(-2) +
            (3*(1+B)**2*(t/t_bar)**2-(3+6*B+14*B**2+6*B**3+3*B**4))
        )
...
def K_p(t, t_int, B, t_bar):
    return (t_bar/t)**2 * H_m(t, t_int, B, t_bar)
...
def K_m(t, t_int, B, t_bar):
    return (1-B**2)**2 * t_bar**4/(t**2*t_int**2) * \
        H_p((1-B**2)*t_bar**2/t, (1-B**2)*t_bar**2/t_int, B, t_bar)
